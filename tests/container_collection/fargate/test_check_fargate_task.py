...
def make_describe_tasks_response(status: Optional[str], exit_code: Optional[int]):
    if status is None:
        return {"tasks": []}
    if status == "STOPPED":
...
class TestCheckFargateTask(unittest.TestCase):
    ...
    @mock.patch.object(
        sys.modules["container_collection.fargate.check_fargate_task"], "sleep", lambda _: None
    )
